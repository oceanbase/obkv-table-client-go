...
func TestObPartitionLocation(t *testing.T) {
	l := &obPartitionLocation{}
	assert.Equal(t, "obPartitionLocation{leader:nil, replicas:[]}", l.String())
	...
	assert.Equal(t, r1, l.getReplica(ConsistencyStrong))
	assert.Equal(t, r1, l.getReplica(ConsistencyWeak))
	assert.Equal(t, "obPartitionLocation{leader:obReplicaLocation{addr:ObServerAddr{ip:127.0.0.1, sqlPort:2001, svrPort:2000}, info:obServerStatus{stopTime:0, status:active}, role:1, replicaType:0}, replicas:[obReplicaLocation{addr:ObServerAddr{ip:127.0.0.1, sqlPort:2001, svrPort:2000}, info:obServerStatus{stopTime:0, status:active}, role:1, replicaType:0}, obReplicaLocation{addr:ObServerAddr{ip:127.0.0.1, sqlPort:2001, svrPort:2000}, info:obServerStatus{stopTime:0, status:active}, role:2, replicaType:0}]}", l.String())
}
