...
func (mcs *MockCodec) GetCloseChan() *chan struct{} {
	return &mcs.CloseChan
}
...
func MockTCPServer(t *testing.T, lis net.Listener, doneChan chan struct{}) {
	rpcSrv, err := obkvrpc.NewServer(RoutinePoolSize, new(chan struct{}))
	...
	for {
		...
		codec := MockCodec{Conn: conn, RunTime: 0, Wg: wg, Test: t, CloseChan: make(chan struct{})}
		wg.Add(1)
