...
const (
	autoIncRowkeyNotFillTableTableName       = "autoIncRowkeyNotFillTable"
	autoIncRowkeyNotFillTableCreateStatement = "create table if not exists autoIncRowkeyNotFillTable(`c1` bigint(20) not null auto_increment, c2 bigint(20) not null, c3 varchar(20) default 'hello', c4 bigint(20) default 0, primary key (`c1`, `c2`)) partition by hash(c2) partitions 15;"
)

func TestAuto_IncRowkeyNotFill(t *testing.T) {
	tableName := autoIncRowkeyNotFillTableTableName
	defer test.DeleteTable(tableName)

	// test insert.
	// test auto inc value.
	rowKey := []*table.Column{table.NewColumn("c2", int64(1))}
